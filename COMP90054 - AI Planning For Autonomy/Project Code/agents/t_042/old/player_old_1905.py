...
class myAgent(Agent):
    ...
    def SelectAction(self, actions: Iterable, s: GameState):
        ...
        for a in actions:
            ...
            h = 100*self.RingHeuristic(ns)\
                + 2*(self.ChainHeuristic(ns,0) - self.ChainHeuristic(ns,1))\
                + 1.2*self.CornerHeuristic(ns)
            if h>best_h:
                best_h = h
                best_action = a
    ...
    def ChainHeuristic(self,s: GameState,
                            ...
                            pts: dict = {0:0, 1:0, 2:1, 3:3, 4:5, 5:6, 6:6,7:6,8:6,9:6,10:6}):
        '''
        ...
        for p in lines_of_interest:
            ...
            # for st in re.findall(f'[{counter}]*{str(EMPTY)*1}[{opponent_counter*1}]*{ring}'
            #                   + f'|{ring}[{opponent_counter*1}]*{str(EMPTY)*1}[{counter}]*',p_str):
            #     tot_mark+=0.5*pts[max(st.count(counter) + st.count(opponent_counter),5)-1]
            # # EMPTY-nM_opponent-R-nM
            # for st in re.findall(f'{str(EMPTY)*1}[{opponent_counter*1}]*{ring}[{counter}]*'
            #                   + f'|[{counter}]*{ring}[{opponent_counter*1}]*{str(EMPTY)*1}',p_str):
            #     tot_mark+=0.5*pts[max(st.count(counter) + st.count(opponent_counter),5)-1]
        return tot_mark
    ...
    def Cornered(self, s: GameState, loc: tuple):
        '''
