...
class myAgent(Agent):
    ...
    def SelectAction(self, actions: Iterable, s: GameState):
        ...
        # If there's a ring win move available, use greedy as MCTS would go to terminal state
        if any(a['type']=='place, move, remove' for a in actions):
            # Greedy search
            return self.Greedy(s,actions,start)
        # Otherwise run MCTS
        else: return MCTS(Node(self,s)).search()
    ...
    def Reward(self, ns: GameState):
        '''Calculate the reward of a given new state'''
        return 100*self.RingHeuristic(ns)\
    ...
    def ChainHeuristic(self,s: GameState,
                            ...
                            pts: dict = {0:0, 1:0, 2:1, 3:3, 4:5, 5:50}):
        '''
